...
## 5. Keep the original retrieval files

The `.tsv` and `.fasta` files generated by the retrieval tool are the original outputs of this step.

Keep both files unchanged.

The TSV will be curated in Activity 02, while the original FASTA will be used to recover the sequences corresponding to the retained accessions.
...
- a TSV table of candidate proteins from the workshop taxa;
- the corresponding original FASTA file;
